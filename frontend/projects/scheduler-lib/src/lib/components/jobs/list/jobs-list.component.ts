...
export class JobsListComponent implements OnInit, AfterViewInit {
  ...
  jobs = signal<IJob[]>([]);
  dataSource: any = signal<MatTableDataSource<IJob>>(new MatTableDataSource<IJob>([]));
  ...
  private jobService = inject(SchedulerApiService);
  ...
  private loadData() {
    ...
    this.jobService.getAllJobs(this.searchValue(), 0, this.pageSize(), this.getSortValue())
    ...
    this.sort.sortChange
      .pipe(
        switchMap(() => {
          ...
          return this.jobService.getAllJobs(this.searchValue(), 0, this.pageSize(), this.getSortValue());
        }),
        map((data) => {
          this.isLoadingResults.set(false);
          return data;
  ...
  deleteJobActionResult = (action: any, job: any, index: any) => {
    if (action) {
      this.jobService.deleteJob(job.jobName, job.jobGroup)
        .pipe(takeUntilDestroyed(this.destroyRef))
        .subscribe({
          next: (resp: any) => {
  ...
  applyFilter(searchValue: any) {
    ...
    this.isLoadingResults.set(true);
    this.jobService
      .getAllJobs(this.searchValue(), this.currentPage() * this.pageSize(), this.pageSize(), this.getSortValue())
      .pipe(takeUntilDestroyed(this.destroyRef))
      .subscribe({
        next: (data) => {
          this.isLoadingResults.set(false);
