...
import { IJob, JobData } from '../../../models/job.model';
...
export class JobDetailsComponent implements OnInit {
  ...
  // Data sources
  dataSourceJobData = computed(() => of(this.jobData()));
  dataSourceTriggers = computed(() => of(this.triggers()));
  ...
  public translate = inject(DefaultSchedulerTranslationService);
  ...
  private initializeExecutionHistoryColumns(): void {
    this.executionHistorycolumns = [
      {
        column: 'triggerName',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.TRIGGER-NAME', 'Trigger Name'),
        sort: true,
        filter: true,
      ...
      {
        column: 'triggerGroup',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.TRIGGER-GROUP', 'Trigger Group'),
        sort: false,
        filter: true,
        type: ListColumnType.String,
      },
      {
        column: 'jobClass',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.JOB-CLASS', 'Job Class'),
        sort: false,
        filter: true,
        type: ListColumnType.String,
      },
      {
        column: 'status',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.STATUS', 'Status'),
        sort: false,
        filter: true,
        type: ListColumnType.String,
      },
      {
        column: 'duration',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.DURATION', 'Duration'),
        sort: false,
        filter: true,
        type: ListColumnType.String,
      },
      {
        column: 'firedTime',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.FIRE-TIME', 'Fire Time'),
        sort: false,
        filter: true,
      ...
      {
        column: 'finishedTime',
        label: this.translate.instant('EXECUTION-HISTORY.FIELDS.FINISHED-TIME', 'Finished Time'),
        sort: false,
        filter: true,
        type: ListColumnType.Date,
      },
  ...
  getJob(jobName: string, jobGroup: string): void {
    ...
    this.schedulerApiService.getJob(jobName, jobGroup).subscribe({
      next: (job) => {
        this.job.set(job);
        this.setJobData(job);
  ...
  getJobExecutionHistory(jobName: string, jobGroup: string): void {
    this.schedulerApiService.getJobExecutionHistoryByJob(jobName, jobGroup, [], 0, 30, "").subscribe({
  ...
  onSubmit(): void {
    ...
    this.schedulerApiService
      .updateJob(newJob, newJob['jobName'], newJob['jobGroup'])
      .pipe(first())
      .subscribe({
        next: (data) => {
  ...
  getErrorMessage(fieldName: string): string {
    ...
    if (field?.hasError('maxlength')) {
      ...
      return this.translate.instant('SCHEDULER-GENERAL.ERRORS.LENGTH-EXCEEDING', `Maximum length is ${maxLength} characters`);
    }
    return '';
  }
}
