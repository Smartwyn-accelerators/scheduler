import { Component, OnInit, Output, EventEmitter, Input, ElementRef, ViewChild, signal, inject } from '@angular/core';
...
export class ListFiltersComponent implements OnInit {
   ...
   */
  setFilterableFields() {
    this.columnsList.forEach((column: IListColumn) => {
      if (column.filter) {
  ...
  search(): void {
    if (this.field) {
      ...
      this.filterFields.splice(
        this.filterFields.findIndex((filterField: IListColumn) => filterField === this.field),
        1
      );
    ...
    switch (formData.operator) {
      case operatorType.Contains:
        this.selectedDisplayFilterFields.push(
          formData.fieldName +
            ': ' +
            this.translate.instant('LIST-FILTERS.FIELD-CRITERIA-DISPLAY.CONTAINS', 'contains') +
            ' "' +
      ...
      case operatorType.Equals:
        this.selectedDisplayFilterFields.push(
          formData.fieldName +
            ': ' +
            this.translate.instant('LIST-FILTERS.FIELD-CRITERIA-DISPLAY.EQUALS', 'equals') +
            ' "' +
            formData.searchValue +
            '"'
        );
      ...
      case operatorType.NotEqual:
        this.selectedDisplayFilterFields.push(
          formData.fieldName +
            ': ' +
            this.translate.instant('LIST-FILTERS.FIELD-CRITERIA-DISPLAY.NOT-EQUAL', 'not equal') +
            ' "' +
            formData.searchValue +
            '"'
        );
      ...
      case operatorType.Range:
        ...
        if (formData.startingValue) {
          displayField =
            displayField +
            ' ' +
            this.translate.instant('LIST-FILTERS.FIELD-CRITERIA-DISPLAY.FROM', 'from') +
            ' "' +
        ...
        if (formData.endingValue) {
          displayField =
            displayField +
            ' ' +
            this.translate.instant('LIST-FILTERS.FIELD-CRITERIA-DISPLAY.TO', 'to') +
            ' "' +
  ...
  selected(event: MatAutocompleteSelectedEvent): void {
    console.log('Select Value Event :', event);
    //getting Icolumnfield object for selected field
    this.field = this.filterFields?.find((x: IListColumn) => x.searchColumn == event.option.value);
    this.basicFilterForm.get('fieldName')?.setValue(this.field?.searchColumn);
    this.setOperators();
  ...
  remove(field: string, index: number): void {
    ...
    let filterField:any = this.columnsList.find((x: IListColumn) => {
   ...
   */
  setOperators() {
    this.operators = Object.keys(operatorType).map((k) => operatorType[k as keyof typeof operatorType]);
