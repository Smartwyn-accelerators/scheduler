...
public class CGenClassLoader extends ClassLoader {
	...
	public List<Class<?>> findClassesFromClassLoader(String packageName) throws ClassNotFoundException {
		try {
			...
			while (loader != null) {
				for (URL url : ((URLClassLoader) loader).getURLs()) {
					...
					if (Files.isDirectory(path)) {
						Map<String, String> classFiles = CGenClassLoader.retrieveClasses(path, packageName);
						for (String className : classFiles.keySet()) {
							Class<?> cs = loader.loadClass(className);
							Class<?>[] interfaces = cs.getInterfaces();
							for (Class<?> iface : interfaces) {
								if (iface.isAssignableFrom(org.quartz.Job.class)) {
									classes.add(cs);
								}
							}
						}
			...
			e.printStackTrace();
			throw new ClassNotFoundException("Error while retrieving classes from classloader", e);
		}
	}
